...
####TODO verify for non1000K
def get_UserData():
    col_names = ["user", "age", "gender", "occupation", "PostCode"]
    df = pd.read_csv(base_path + 'u.user', sep='|', header=None, names=col_names, engine='python')
    del df["PostCode"]
    df["user"] -= 1
...
def get_ItemData():
    col_names = ["movieid", "movietitle", "releasedate", "videoreleasedate", "IMDbURL"
        ...
        , "War", "Western"]
    df = pd.read_csv(base_path + 'u.item', sep='|', header=None, names=col_names, engine='python')
...
#TODO ITEMDATA = get_ItemData100k()
ITEMDATA = get_ItemData()

dictUsers = load_data(base_path + "UserDict.dat")
df_train = load_data(base_path + "RankData.dat")
print(len(df_train))
