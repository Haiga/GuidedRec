...
def data_preparation(ratings_file, output_file, sep="\t"):
    ...
    with open(ratings_file) as fi:
        with open(output_file, "w+") as fo:
            for line in fi:
                line_splitted = json.loads(line.strip())
                user_id = line_splitted["reviewerID"]
                item_id = line_splitted["asin"]
                rating = line_splitted["overall"]
                timestamp = line_splitted["unixReviewTime"]
...
if __name__ == '__main__':
    ...
    for dataset in ['music']:
        ...
        data_preparation(f'src/Data/{dataset}/music.json', f'src/Data/{dataset}/u.data', ",")
        e = time.time()
