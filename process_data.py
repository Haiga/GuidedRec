...
def data_preparation(ratings_file, ouput_path, sep="\t"):
    # all_ratings = pd.read_csv(ratings_file, sep=sep, names=["user", "item", "rating", "timestamp"])
    all_ratings = pd.read_csv(ratings_file, sep=sep, names=["user", "item", "rating", "timestamp"]).groupby("user").filter(
        lambda x: len(x) > 10)
    # pd.read_csv('src/Data/music/u.data', sep=',', names=["user", "item", "rating", "timestamp"]).groupby("user").filter(
    #     lambda x: len(x) > 5)
    ...
    for i in range(USER_NUM):
        ...
        if len(validation_interaction) != 0:
            # try:
