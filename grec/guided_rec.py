...
def GuidedRec(train, ItemData=False, UserData=False, Graph=True, lr=0.00003, ureg=0.0, ireg=0.0):
    ...
    ## Logloss
    cost, train_op = optimization(inferSig, infers_drop, regularizer, rate_batch, learning_rate=lr, reg=0.09, device=DEVICE)
    ...
    with tf.Session(config=config) as sess:
        ...
        textTrain_file = open(
            "./Output/" + now.strftime('%Y%m%d%H%M%S') + '_Graph' + "_" + str(SURROGATE) + "_" + str(SEED) +name_exec+ ".txt",
            "w", newline='')
                ...
                if NDCG10 > bestndcg10:
                    ...
                    print(
                        "NEWBEST:{:3d},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f}".format(i // samples_per_batch,
                                                                                                 ...
                                                                                                 HitRatio20,
                                                                                                 NDCG5, NDCG10, NDCG20))
                    with open(name_exec + "ml100k-measures.csv", "w+") as fo:
