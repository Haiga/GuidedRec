...
def optNDCG(NDCGscore, varlist, learning_rate=0.00003, reg=0.1, device="/cpu:0"):
    with tf.device(device):
        costNDCG = -NDCGscore
        train_NDCG = tf.contrib.opt.AdamWOptimizer(WGT_DECAY_NDCG, learning_rate=LR_NDCG).minimize(costNDCG, var_list=varlist)
    return train_NDCG
...
def optLoss(NDCGscore, NDCGTrue, learning_rate=0.00003, reg=0.1, device="/cpu:0"):
    with tf.device(device):
        ...
        train_LOSS = tf.contrib.opt.AdamWOptimizer(WGT_DECAY_SURROGATE, learning_rate=LR_SURROGATE).minimize(costLOSS)
...
def optLoss2(NDCGscore, NDCGTrue, learning_rate=0.00003, reg=0.1, device="/cpu:0"):
    with tf.device(device):
        ...
        tempgrads2 = tf.gradients(tempgrads1[0], tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Surrogate'),
                                  unconnected_gradients='zero')
...
def GuidedRec(train, ItemData=False, UserData=False, Graph=True, lr=0.00003, ureg=0.0, ireg=0.0):
    ...
    with tf.variable_scope("Surrogate"):
        ...
        l4n = tf.layers.dense(inputs=l31n, units=8, reuse=True, trainable=False, activation=tf.nn.tanh,
                              kernel_initializer=tf.random_normal_initializer(stddev=0.01), name="l4")
        NDCGScore_with_recsys = tf.layers.dense(inputs=l4n, units=1, reuse=True, trainable=False, activation=tf.nn.tanh,
                                                kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                                name="NDCGScore")
    ...
    with tf.Session(config=config) as sess:
        ...
        textTrain_file = open(
            "./Output/" + now.strftime('%Y%m%d%H%M%S') + '_Graph' + "_" + str(SURROGATE) + "_" + str(SEED) + ".txt",
            "w", newline='')
        ...
        for i in range(EPOCH_MAX * samples_per_batch):
            ...
            if i % samples_per_batch == 0:
                ...
                print(
                    "{:3d},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f}".format(i // samples_per_batch, train_err,
                                                                                     train_err1, train_err2, train_err3,
                                                                                     HitRatio5, HitRatio10, HitRatio20,
                                                                                     NDCG5, NDCG10, NDCG20))
                ...
                if NDCG10 > bestndcg10:
                    ...
                    print(
                        "NEWBEST:{:3d},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f}".format(i // samples_per_batch,
                                                                                         ...
                                                                                         HitRatio20,
                                                                                         NDCG5, NDCG10, NDCG20))
                    with open("ml1m-measures.csv", "w+") as fo:
